...
def read_medication(medication_id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_active_user)):
...
def read_user_medication(user_id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_active_user)):
...
def read_medications(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: schemas.User = Depends(get_current_active_user)):
    medications = crud.get_medications(db, skip=skip, limit=limit)
    return medications

@router.put("/", response_model=schemas.Medication)
def create_medication(medication: schemas.MedicationCreate, user_id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(verify_role(UserRole.DOCTOR))):
...
def update_medication(medication_id: int, medication: schemas.MedicationUpdate, db: Session = Depends(get_db), current_user: schemas.User = Depends(verify_role(UserRole.DOCTOR))):
    db_medication = crud.get_medication(db, medication_id=medication_id)
    if db_medication is None:
        raise HTTPException(status_code=404, detail="Medication not found")
...
def delete_medication(medication_id: int, db: Session = Depends(get_db), current_user: schemas.User = Depends(verify_role(UserRole.DOCTOR))):
    db_medication = crud.get_medication(db, medication_id=medication_id)
    if db_medication is None:
        raise HTTPException(status_code=404, detail="Medication not found")
